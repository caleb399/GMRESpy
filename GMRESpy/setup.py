...
class NinjaBuild(SetuptoolsBuildExt):
    ...
    def build_extension(self, ext):
        ...
        lib_install_dir = os.path.join(module_install_dir, 'lib')
        if not os.path.exists(lib_install_dir):
            try:
                os.mkdir(lib_install_dir)
            except OSError as e:
                print(f'Could not create lib directory ({lib_install_dir}).')
                print(f'Original error message: {e}')
        try:
            shutil.move(target_lib, os.path.join(lib_install_dir, "dpcpp_gmres.pyd"))
        except shutil.Error as e:
            print(f'Could not move target library ({target_lib}) to install directory ({lib_install_dir}).')
            print(f'Original error message: {e}')
