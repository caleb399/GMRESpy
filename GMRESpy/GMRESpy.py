...
def gmres(A, x0, b, n, rtol = 1e-6, restart = 40, max_iter = 1000, device = "DEFAULT", show_progress = False):
	...
	try:
		...
		print(e)
		print(f'\nFunction arguments did not pass validation; see above error message for details. (File: {__file__})')
		exit()
	else:
		# params pass check, so call DPC++ gmres function
		solution, final_error, status_code = dpcpp_gmres.gmres(
			A = validated_params.A,
			x0 = validated_params.x0,
			b = validated_params.b,
			n = validated_params.n,
			rtol = validated_params.rtol,
			restart = validated_params.restart,
